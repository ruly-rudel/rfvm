...
#define TYPE_VEC     ((rfval_t){ .imm.main = IMM_T, .imm.sub = VEC_T,    .imm.val   = 0 })
...
INLINE(bool    vectorp(rfval_t x),  x.imm.main == PTR_T  &&  ((rfval_t*)x.ptr)->imm.main == IMM_T && ((rfval_t*)x.ptr)->imm.sub == VEC_T)
